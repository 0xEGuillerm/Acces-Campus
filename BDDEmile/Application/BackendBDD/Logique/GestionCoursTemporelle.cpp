...
drogon::Task<> GestionCoursTemporelle::traiterCoursADemarrer() {
    ...
    int64_t TimestampMinuteDebut = (trantor::Date::now().microSecondsSinceEpoch()/60000000)*6000000;
    int64_t TimestampMinuteFin = TimestampMinuteDebut + 6000000;
    ...
    if (!ListeCours.donnee.empty()) {
        for (const auto &coursAjout : ListeCours.donnee) {
            ...
            auto Utilisateurliste = co_await UtilisateurDAO::ChercherUtilisateurParClasse(db, coursAjout.getValueOfNumSalle());
            //renvoie un bool false et un message d'erreur correspondant si probleme
            if (!Utilisateurliste.donnee.empty() || !Utilisateurliste.BoolResultat == false) {
...
drogon::Task<> GestionCoursTemporelle::traiterCoursACloturer() {
    ...
    if (!ListeCours.donnee.empty()) {
        ...
        for (const auto &coursAjout : ListeCours.donnee) {
            ...
            if (!ListeAbsence.donnee.empty() || !ListeAbsence.BoolResultat == false) {
                for (const auto &Absence : ListeAbsence.donnee) {
                    ...
                    CreationRetard.setTempsRetardMin(60);
