...
drogon::Task<ResultatCoro<std::vector<drogon_model::acces_campus_bdd::Classe>>> ClasseDAO::ListeClasse(
    //Alias d'un shared pointeur vers le client postgres (pour gère la connexion)(nécessaire pour utiliser la db au niveau du DAO)
    const DbClientPtr &db) {
    //Mapper de classe / objet <--> SQL
    CoroMapper<drogon_model::acces_campus_bdd::Classe> mappersalle(db);
    //Variable pour le résultat / envoie rien pour la template (monostate)
    ResultatCoro<std::vector<drogon_model::acces_campus_bdd::Classe>> resultat;
    //Gestion des erreurs avec try/catch
    try {
        //Execution commande SQL d'envoie de toutes les classes
